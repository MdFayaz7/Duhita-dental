...
export function FormSection({ step, title, children }) {
  return (
    <fieldset className="grid gap-5 min-w-0 border-t border-line pt-7 first:border-0 first:pt-0">
      <legend className="flex items-center gap-3 mb-5">
        ...
        <span className="font-display text-[24px] text-ink">{title}</span>
      </legend>
      {children}
    </fieldset>
  );
...
export function Chip({ active, onClick, children, disabled }) {
  return (
    <button
      ...
      disabled={disabled}
      aria-pressed={active}
      className={`px-4 py-2 rounded-full border text-[14px] transition-colors disabled:opacity-35 disabled:cursor-not-allowed disabled:line-through ${
